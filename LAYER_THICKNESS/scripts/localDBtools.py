...
#            client = MongoClient(host='atlasdb01.kek.jp', port=27017)
    ...
    def get_aLayerThicknessData(self, sn,  debug):
        stage='PCB_RECEPTION'
        component= self.localDB.component.find_one({'serialNumber': sn})
        if component == None :
            print(sn,"\t","not pulled yet")
        ...
        val=qcStatus.get('QC_results').get(stage).get('LAYER_THICKNESS')
        if debug :
            print("==QC_resutls==")
            pprint.pprint(val)
        ...
        if val == "-1":
            ...
            LayerThickness_json = test['results']
            
            return LayerThickness_json
